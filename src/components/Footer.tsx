      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid lg:grid-cols-12 gap-8 items-start mb-8 pb-8 border-b border-white/5">
          ...
          <div className="lg:col-span-6 flex flex-col gap-4">
            ...
            <p className="text-[11.5px] text-slate-400 leading-relaxed max-w-xl pr-4">
            ...
            <div className="glass-panel rounded-2xl p-3 border border-brand-gold/10 bg-slate-950/40 max-w-md mt-1">
              <span className="text-[9.5px] text-brand-gold font-bold uppercase tracking-wider block mb-1.5">
                ⚡ Join the KKNL Member Newsletter
              </span>
              {subscribed ? (
                <div className="px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-lg text-[10px] font-bold flex items-center gap-1">
                  <Check size={12} />
                ...
                <form onSubmit={handleSubscribe} className="flex gap-1.5">
                  <input
                    ...
                    className="flex-grow px-3 py-1.5 bg-slate-900/60 border border-slate-700/60 rounded-lg text-slate-200 focus:outline-none focus:border-brand-gold text-[10.5px] h-8"
                    required
                  />
                  <button
                    type="submit"
                    className="px-4 h-8 rounded-lg bg-brand-gold hover:bg-brand-gold-hover text-brand-blue font-bold text-[10.5px] flex items-center gap-1 transition-all active:scale-95 shadow-sm"
                  >
                    <Send size={10} />
                    Join
                  </button>
          ...
          <div className="lg:col-span-6 grid grid-cols-2 gap-6">
            ...
            {/* Sitemap Navigation */}
            <div>
              <h4 className="font-heading font-black text-[11px] text-white uppercase tracking-wider mb-3.5 border-l-2 border-brand-gold pl-2">
                Quick Sitemap
              </h4>
              <ul className="space-y-1.5">
                {navLinks.slice(0, 4).map((link) => (
                  <li key={link.name}>
                    <a
                      href={link.href}
                      className="text-[11px] text-slate-400 hover:text-brand-gold hover:translate-x-1 flex items-center transition-all duration-150"
                    >
                      {link.name}
                    </a>
                  </li>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8 relative">
          ...
          <div className="glass-panel rounded-2xl p-3 border border-brand-gold/15 bg-slate-950/30 flex items-start gap-3">
            ...
            <div>
              <span className="text-[8px] uppercase font-bold text-slate-400 block tracking-wider">MCA Registration</span>
              <span className="text-[10.5px] text-white font-extrabold block mt-0.5">Govt. Rules Compliant</span>
              <span className="text-[9px] text-slate-500 block mt-0.5 leading-none">Nidhi Rules 2014</span>
            </div>
          </div>
          ...
          <div className="glass-panel rounded-2xl p-3 border border-brand-gold/15 bg-slate-950/30 flex items-start gap-3">
            ...
            <div>
              <span className="text-[8px] uppercase font-bold text-slate-400 block tracking-wider">Legal Entity</span>
              <span className="text-[10.5px] text-white font-extrabold block mt-0.5">Registered Nidhi Ltd</span>
              <span className="text-[9px] text-brand-gold font-bold block mt-0.5 leading-none">U65991MH2026PLC999999</span>
            </div>
          </div>
          ...
          <div className="glass-panel rounded-2xl p-3 border border-brand-gold/15 bg-slate-950/30 flex items-start gap-3">
            ...
            <div>
              <span className="text-[8px] uppercase font-bold text-slate-400 block tracking-wider">Investment Safety</span>
              <span className="text-[10.5px] text-white font-extrabold block mt-0.5">100% Protected Assets</span>
              <span className="text-[9px] text-slate-500 block mt-0.5 leading-none">Physical vault custody</span>
            </div>
          </div>
          ...
          <div className="glass-panel rounded-2xl p-3 border border-brand-gold/15 bg-slate-950/30 flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <span className="text-[8px] uppercase font-bold text-slate-400 block tracking-wider">Sangli HQ Address</span>
              <span className="text-[10px] text-white font-extrabold block mt-0.5 leading-tight">Yashwantnagar Branch</span>
              <p className="text-[9px] text-slate-400 truncate mt-0.5 leading-none" title="CS 1642, Near Ganpati Temple, Sangli">
                C.S. No. 1642, Sangli - 416416
              </p>
              <div className="flex gap-2.5 mt-1 border-t border-white/5 pt-1 text-[9px]">
        ...
        <div className="text-[8.5px] text-slate-500 leading-relaxed mb-4 bg-slate-950/40 p-3 rounded-xl border border-white/5 shadow-inner">
        ...
        {/* Copyright */}
        <div className="flex flex-col sm:flex-row justify-between items-center text-[10px] text-slate-500 pt-1.5 border-t border-white/5">
          <p>© {currentYear} Kusumbha Kalyan Nidhi Limited (KKNL). All rights reserved.</p>
          <p className="mt-2 sm:mt-0 flex gap-3 text-[9.5px]">
